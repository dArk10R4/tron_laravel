        ...
        <div class="col-xl-3">
            <div class="sp_user_sidebar h-100">
                ...
                <ul class="user-action-list">
                    <li>
                        <a href="{{ route('admin.login.user', $user->id) }}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="fas fa-link mr-1"></i>
                            {{ 'Login As User' }}</a>
                    </li>
                    ...
                    <li>
                        <a href="{{ route('admin.ticket.index', ['user' => $user->id]) }}" class="btn btn-sm btn-outline-primary"><i class="fas fa-link mr-1"></i>
                            {{ 'User Ticket' }}</a>
                    </li>
                    ...
                    <li>
                        <form action="{{ route('admin.user.delete', $user->id) }}" method="post">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-primary"><i class="fas fa-link mr-1"></i>
                                {{ 'Delete User' }}</button>
                        </form>
                    </li>
                </ul>
            ...
            <div class="sp_user_details">
                <ul class="nav nav-tabs sp_nav_tabs" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="overview-tab" data-toggle="tab" data-target="#overview" type="button" role="tab" aria-controls="overview" aria-selected="true">{{__('Overview')}}</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="settings-tab" data-toggle="tab" data-target="#settings" type="button" role="tab" aria-controls="settings" aria-selected="false">{{__('Settings')}}</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="balance-tab" data-toggle="tab" data-target="#balance" type="button" role="tab" aria-controls="balance" aria-selected="false">Balance</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="referral-tab" data-toggle="tab" data-target="#referral" type="button" role="tab" aria-controls="referral" aria-selected="false">{{__('referral')}}</button>
                    </li>
                </ul>
                        ...
                        <form action="{{ route('admin.user.update', $user->id) }}" method="post">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <label>{{ __('city') }}</label>
                                    <input type="text" name="city" class="form-control form_control" value="{{ @$user->address->city }}">
                                </div>

                                <div class="col-md-4 mb-3">

                                    <label>{{ __('zip') }}</label>
                                    <input type="text" name="zip" class="form-control form_control" value="{{ @$user->address->zip }}">
                                </div>

                                <div class="col-md-4 mb-3">
                                    <label>{{ __('state') }}</label>
                                    <input type="text" name="state" class="form-control form_control" value="{{ @$user->address->state }}">
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            ...
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" name="email_status" {{ $user->ev ? 'checked' : '' }} class="custom-control-input" id="customSwitch1">
                                                <label class="custom-control-label" for="customSwitch1">Active/Deactive</label>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <p for="">{{ __('SMS Verified') }}</p>
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" name="sms_status" {{ $user->sv ? 'checked' : '' }} class="custom-control-input" id="customSwitch2">
                                                <label class="custom-control-label" for="customSwitch2">Active/Deactive</label>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <p for="">{{ __('KYC Verified') }}</p>
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" name="kyc_status" {{ $user->kyc ? 'checked' : '' }} name="email_status" class="custom-control-input" id="customSwitch3">
                                                <label class="custom-control-label" for="customSwitch3">Active/Deactive</label>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <p for="">{{ __('Status') }}</p>
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" name="status" {{ $user->status ? 'checked' : '' }} name="email_status" class="custom-control-input" id="customSwitch4">
                                                <label class="custom-control-label" for="customSwitch4">Active/Deactive</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="tab-pane fade pt-4" id="balance" role="tabpanel" aria-labelledby="balance-tab">
                        <form action="{{ route('admin.user.balance.update', $user->id) }}" method="post">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="number" class="form-control" name="balance" min="1" placeholder="add Balance">
                        ...
                        <form action="{{ route('admin.user.balance.update', $user->id) }}" method="post">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="number" class="form-control" name="balance" min="1" placeholder="Subtract Balance">
                                                ...
                                                <ul class="sub-child-list step-4">
                                                    ...
                                                    <li class="single-child">
                                                        <p>
                                                            ...
                                                            <span class="mb-0">{{ $ref2->full_name }}</span>
                                                        </p>
                                                    </li>
                                                    @endforeach
                                                </ul>
